...
export default function App() {
  ...
  // A referral mints CRED that nobody paid for, so the grant cannot be driven
  // from the browser. claim_referral_bonus binds the claim to the calling
  // session and uses the unique referrals.referred_id as its idempotency key,
  // which also replaces the read-then-insert race this used to have.
  const processReferral = async (referrerUsername: string) => {
    try {
      ...
      if (error) console.error('[Referral] Processing error:', error.message);
    } catch (err) {
