          ...
          <AnimatePresence mode="wait">
            <motion.div key={activeTab} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="grid gap-6">
              {activeTab === 'cockpit' && <>
                ...
                <GlassPanel><div className="mb-5 flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.3em] text-cyan-300">GhostOps Parallelism</p><h2 className="text-2xl font-black uppercase tracking-[0.12em]">Sub-Agent Tree</h2></div><GitBranch className="h-6 w-6 text-cyan-200" /></div>{subagents.length === 0 ? <div className="rounded-3xl border border-dashed border-white/10 bg-black/35 p-8 text-center"><Ghost className="mx-auto mb-3 h-10 w-10 text-zinc-700" /><p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">No sub-agents have reported in yet.</p></div> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{subagents.map(agent => <motion.div key={agent.id} layout className="rounded-3xl border border-white/10 bg-black/35 p-4"><div className="mb-3 flex items-start justify-between gap-3"><div><span className={cn('rounded-full border px-3 py-1 text-[8px] font-black uppercase tracking-widest', subagentStatusStyles[agent.status])}>{agent.status}</span><p className="mt-2 text-[10px] font-black uppercase tracking-widest text-cyan-100">@{agent.user?.username || agent.user_id.slice(0, 8)}</p></div>{(agent.status === 'queued' || agent.status === 'working' || agent.status === 'awaiting_client') && <button onClick={() => void cancelSubagent(agent)} className="rounded-xl border border-red-400/30 bg-red-500/10 p-2 text-red-200 hover:bg-red-500/20"><Square className="h-4 w-4" /></button>}</div><p className="text-xs font-bold leading-5 text-zinc-200">{agent.objective}</p>{agent.result && <p className="mt-3 line-clamp-3 rounded-xl border border-green-400/10 bg-green-400/[0.04] p-3 text-xs leading-5 text-green-100">{agent.result}</p>}</motion.div>)}</div>}</GlassPanel>
              </>}
