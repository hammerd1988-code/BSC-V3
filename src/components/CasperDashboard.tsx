...
export const CasperDashboard: React.FC = () => {
  ...
  const fetchDashboard = useCallback(async () => {
    try {
      ...
      if (routinesRes.data) setRoutines(routinesRes.data as CasperRoutineRow[]);
      if (skillsRes.data) setSkills(skillsRes.data as CasperSkillRow[]);
  ...
  const saveRoutine = async () => {
    ...
    setSaving(true); setNotice(null);
    try {
      const payload = { name: routineForm.name.trim(), directive: routineForm.directive.trim(), frequency: routineForm.frequency, cron_expression: routineForm.frequency === 'cron' || routineForm.frequency === 'custom' ? routineForm.cron_expression : null, scheduled_time: routineForm.scheduled_time, scheduled_days: routineForm.scheduled_days, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC', is_enabled: true, next_run_at: nextRun(routineForm.frequency, routineForm.scheduled_time), created_by: userUuid, metadata: { owner_id: userUuid } };
  ...
  const updateRoutine = async (routine: CasperRoutineRow, patch: Partial<CasperRoutineRow>) => { const { error } = await supabase.from('casper_routines').update(patch).eq('id', routine.id); if (error) setNotice(error.message); else await fetchDashboard(); };
