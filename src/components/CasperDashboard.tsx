...
interface CasperSubagentRow {
  ...
  created_at: string;
  completed_at: string | null;
  metadata?: { tool_calls?: Array<{ id: string; name: string; ok: boolean; error: string | null; durationMs: number }>; tool_call_count?: number } | null;
...
interface NoticeEntry { id: string; message: string; level: NoticeLevel; }
function inferNoticeLevel(message: string): NoticeLevel {
  const m = message.toLowerCase();
  return /(error|fail|invalid|denied|unable|missing|unavailable)/.test(m) ? 'error' : 'info';
}

const TASK_STATUS_FILTERS: ReadonlyArray<'all' | CasperTaskRow['status']> = ['all', 'pending', 'running', 'completed', 'failed'];
const SUBAGENT_STATUS_FILTERS: ReadonlyArray<'all' | CasperSubagentRow['status']> = ['all', 'queued', 'working', 'awaiting_client', 'completed', 'failed'];
...
const CasperDashboardInner: React.FC = () => {
  ...
  // Wraps the queue with the legacy setNotice(message) / setNotice(null) signature
  // so all 28 existing call sites stay one-line. Errors stay until dismissed; info
  // entries auto-clear after 6s via the timer effect below.
  const setNotice = useCallback((message: string | null) => {
    if (message === null) { setNotices([]); return; }
    setNotices(prev => [...prev, { id: `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, message, level: inferNoticeLevel(message) }].slice(-6));
  }, []);
  const dismissNotice = useCallback((id: string) => setNotices(prev => prev.filter(n => n.id !== id)), []);
  ...
  // Auto-dismiss info-level notices after 6s; errors stay until clicked.
  useEffect(() => {
    const expiring = notices.filter(n => n.level === 'info');
    if (expiring.length === 0) return;
    const timers = expiring.map(n => window.setTimeout(() => dismissNotice(n.id), 6000));
    return () => timers.forEach(clearTimeout);
  }, [notices, dismissNotice]);

  // Alt+1..7 swap tabs; ignored while typing in inputs/textareas/contenteditable.
  useEffect(() => {
  ...
  const filteredActivities = useMemo(() => {
    ...
    return activities.filter(a => `${a.action_type} ${a.description}`.toLowerCase().includes(q));
  ...
  const updateRoutine = async (routine: CasperRoutineRow, patch: Partial<CasperRoutineRow>) => { const safePatch = { ...patch }; const enabledToggle = safePatch.is_enabled; delete safePatch.is_enabled; const dbPatch = toDb(safePatch); if (enabledToggle !== undefined) dbPatch.is_enabled = enabledToggle; const { error } = await supabase.from('casper_routines').update(dbPatch).eq('id', routine.id); if (error) setNotice(error.message); else await fetchDashboard(); };
  ...
  return (
    <div className="relative min-h-screen bg-[#020205] pb-32 text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <AnimatePresence initial={false}>
              {notices.map(entry => (
                <motion.div
                  ...
                  className={cn(
                    'pointer-events-auto flex items-start gap-3 rounded-2xl border p-3 text-xs font-bold backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.4)]',
                    entry.level === 'error'
                      ? 'border-red-300/40 bg-red-950/55 text-red-100'
                      : 'border-cyan-300/30 bg-cyan-950/40 text-cyan-100',
                  )}
                  role={entry.level === 'error' ? 'alert' : 'status'}
                >
                  {entry.level === 'error' ? <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" /> : <Sparkles className="mt-0.5 h-4 w-4 shrink-0" />}
                  <span className="flex-1 leading-5">{entry.message}</span>
                  <button onClick={() => dismissNotice(entry.id)} aria-label="Dismiss notification" className="text-zinc-300 transition hover:text-white"><X className="h-4 w-4" /></button>
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
        ...
        <div className="sticky top-2 z-30 mb-6 flex gap-2 overflow-x-auto rounded-[2rem] border border-white/10 bg-black/55 p-2 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.35)]">
          {TABS.map((tab, idx) => {
            ...
            return (
              <button
                ...
                title={`Alt+${idx + 1}`}
                className={cn(
                  'flex items-center gap-2 rounded-2xl px-4 py-3 text-[10px] font-black uppercase tracking-[0.2em] transition',
                  isActive ? 'bg-cyan-300/15 text-cyan-100 shadow-[0_0_24px_rgba(0,229,255,0.16)]' : 'text-zinc-500 hover:bg-white/5 hover:text-white',
                )}
              >
                <Icon className="h-4 w-4" />
                <span>{tab.replace(/_/g, ' ')}</span>
                <span className={cn('ml-1 rounded-md px-1.5 py-0.5 text-[8px] font-black tracking-widest', isActive ? 'bg-cyan-300/20 text-cyan-200' : 'bg-white/5 text-zinc-600')}>{idx + 1}</span>
              </button>
            );
          })}
        </div>
            ...
            <motion.div key={activeTab} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="grid gap-6">
              {activeTab === 'cockpit' && <>
                <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
                  ...
                  <GlassPanel><div className="mb-5 flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.3em] text-red-300">Direct Command</p><h2 className="text-2xl font-black uppercase tracking-[0.12em]">Operator Console</h2></div><Command className="h-6 w-6 text-red-300" /></div><textarea value={directCommand} onChange={event => setDirectCommand(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && (event.metaKey || event.ctrlKey) && directCommand.trim() && !saving) { event.preventDefault(); void executeCommand(); } }} rows={5} placeholder="Issue Casper an immediate instruction... (\u2318/Ctrl + Enter to execute)" className="w-full resize-none rounded-3xl border border-white/10 bg-black/45 p-4 text-sm leading-6 text-white outline-none transition focus:border-cyan-300/50" /><button disabled={!directCommand.trim() || saving} onClick={() => void executeCommand()} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl border border-red-300/35 bg-red-500/15 px-4 py-4 text-xs font-black uppercase tracking-[0.24em] text-red-100 transition hover:bg-red-400/20 disabled:opacity-40">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4" />} Execute Directive <span className="ml-2 text-[8px] font-black tracking-widest text-red-200/70">\u2318/Ctrl+\u23CE</span></button><pre className="mt-4 max-h-60 overflow-auto rounded-2xl border border-white/10 bg-black/60 p-4 text-xs leading-5 text-cyan-100 whitespace-pre-wrap">{consoleOutput || 'Console armed. Responses from Casper backend will appear here.'}</pre></GlassPanel>
                </div>
                <div className="grid gap-6 lg:grid-cols-5">{[['Open', taskStats.pending + taskStats.running, Terminal], ['Completed', taskStats.completed, CheckCircle2], ['Routines', routines.filter(r => r.enabled).length, CalendarClock], ['Connected APIs', connectedIntegrations.length, KeyRound], ['Dev Agent', '14 tools', Cpu]].map(([label, value, Icon]: any) => <GlassPanel key={label}>{label === 'Dev Agent' ? <a href="/terminal" className="block group"><Icon className="mb-3 h-6 w-6 text-green-400 group-hover:text-green-300 transition-colors" /><p className="text-3xl font-black text-white group-hover:text-green-300 transition-colors">{value}</p><p className="mt-1 text-[9px] font-black uppercase tracking-widest text-zinc-500 group-hover:text-green-400/60 transition-colors">{label}</p><p className="mt-2 text-[8px] font-black uppercase tracking-widest text-green-400/50">Open Terminal</p></a> : <><Icon className="mb-3 h-6 w-6 text-cyan-200" /><p className="text-3xl font-black text-white">{value}</p><p className="mt-1 text-[9px] font-black uppercase tracking-widest text-zinc-500">{label}</p></>}</GlassPanel>)}</div>
                <GlassPanel><div className="mb-5 flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.3em] text-cyan-300">Bot Ops Parallelism</p><h2 className="text-2xl font-black uppercase tracking-[0.12em]">Sub-Agent Tree</h2></div><GitBranch className="h-6 w-6 text-cyan-200" /></div>
<div className="mb-4 flex flex-wrap items-center gap-2">{SUBAGENT_STATUS_FILTERS.map(status => { const count = status === 'all' ? subagents.length : subagents.filter(a => a.status === status).length; const isActive = subagentStatusFilter === status; return <button key={status} onClick={() => setSubagentStatusFilter(status)} className={cn('flex items-center gap-2 rounded-full border px-3 py-1.5 text-[9px] font-black uppercase tracking-widest transition', isActive ? 'border-cyan-300/40 bg-cyan-300/15 text-cyan-100' : 'border-white/10 bg-black/35 text-zinc-500 hover:bg-white/5 hover:text-white')}><span>{status.replace(/_/g, ' ')}</span><span className={cn('rounded-md px-1.5 py-0.5 text-[8px]', isActive ? 'bg-cyan-300/20 text-cyan-200' : 'bg-white/5 text-zinc-600')}>{count}</span></button>; })}</div>
{filteredSubagents.length === 0 ? <div className="rounded-3xl border border-dashed border-white/10 bg-black/35 p-8 text-center"><Ghost className="mx-auto mb-3 h-10 w-10 text-zinc-700" /><p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">{subagents.length === 0 ? 'No sub-agents have reported in yet.' : `No sub-agents match the \u201C${subagentStatusFilter.replace(/_/g, ' ')}\u201D filter.`}</p>{subagents.length === 0 && <p className="mt-2 text-[9px] uppercase tracking-widest text-zinc-700">Spawn one from a directive that includes \u201Cspin up sub-agents\u201D.</p>}</div> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{filteredSubagents.map(agent => <motion.div key={agent.id} layout className="rounded-3xl border border-white/10 bg-black/35 p-4"><div className="mb-3 flex items-start justify-between gap-3"><div><span className={cn('rounded-full border px-3 py-1 text-[8px] font-black uppercase tracking-widest', subagentStatusStyles[agent.status])}>{agent.status}</span><p className="mt-2 text-[10px] font-black uppercase tracking-widest text-cyan-100">@{agent.user?.username || agent.user_id.slice(0, 8)}</p></div>{(agent.status === 'queued' || agent.status === 'working' || agent.status === 'awaiting_client') && <button onClick={() => void cancelSubagent(agent)} className="rounded-xl border border-red-400/30 bg-red-500/10 p-2 text-red-200 hover:bg-red-500/20"><Square className="h-4 w-4" /></button>}</div><p className="text-xs font-bold leading-5 text-zinc-200">{agent.objective}</p>{agent.result && <p className="mt-3 line-clamp-3 rounded-xl border border-green-400/10 bg-green-400/[0.04] p-3 text-xs leading-5 text-green-100">{agent.result}</p>}{agent.metadata?.tool_calls && agent.metadata.tool_calls.length > 0 && <div className="mt-2 rounded-xl border border-white/[0.06] bg-white/[0.02] p-2"><p className="mb-1 text-[8px] font-black uppercase tracking-widest text-zinc-500">Tool calls ({agent.metadata.tool_calls.length})</p>{agent.metadata.tool_calls.map((tc, i) => <p key={tc.id || i} className="text-[9px] text-zinc-400"><span className={tc.ok ? 'text-emerald-400' : 'text-red-400'}>{tc.ok ? '✓' : '✗'}</span> <span className="font-mono">{tc.name}</span> <span className="text-zinc-600">({tc.durationMs}ms)</span>{tc.error && <span className="ml-1 text-red-400/70">— {tc.error}</span>}</p>)}</div>}</motion.div>)}</div>}</GlassPanel>
              </>}

              {activeTab === 'core' && <GlassPanel><div className="mb-5 flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.3em] text-cyan-300">Configurable Intelligence</p><h2 className="text-2xl font-black uppercase tracking-[0.12em]">Cognitive Core</h2></div><button onClick={() => void saveCore()} disabled={saving} className="rounded-2xl border border-cyan-300/30 bg-cyan-400/10 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-cyan-100"><Save className="mr-2 inline h-4 w-4" />Save Core</button></div><div className="grid gap-4 lg:grid-cols-[0.4fr_0.6fr]"><div className="grid gap-2">{Object.keys(core).map(section => <button key={section} onClick={() => setExpandedCore(section)} className={cn('rounded-2xl border px-4 py-3 text-left text-[10px] font-black uppercase tracking-widest', expandedCore === section ? 'border-cyan-300/30 bg-cyan-400/10 text-cyan-100' : 'border-white/10 bg-black/35 text-zinc-500')}>{section.replace(/_/g, ' ')}</button>)}</div><div className="rounded-3xl border border-white/10 bg-black/35 p-5">{expandedCore === 'personality_traits' && <div className="grid gap-4">{Object.entries(core.personality_traits).map(([key, value]) => <label key={key} className="block"><div className="mb-2 flex justify-between text-[10px] font-black uppercase tracking-widest text-zinc-400"><span>{key.replace(/_/g, ' ')}</span><span className="text-cyan-100">{value}</span></div><input type="range" min={0} max={100} value={value} onChange={e => setCore(prev => ({ ...prev, personality_traits: { ...prev.personality_traits, [key]: Number(e.target.value) } as CognitiveCoreConfig['personality_traits'] }))} className="w-full accent-cyan-300" /></label>)}</div>}{expandedCore === 'knowledge_domains' && <div className="grid gap-3 sm:grid-cols-2">{Object.entries(core.knowledge_domains).map(([key, value]) => <TogglePill key={key} active={Boolean(value)} label={key.replace(/_/g, ' ')} onClick={() => setCore(prev => ({ ...prev, knowledge_domains: { ...prev.knowledge_domains, [key]: !value } }))} />)}</div>}{expandedCore === 'response_style' && <div className="grid gap-3"><input value={core.response_style.tone} onChange={e => setCore(prev => ({ ...prev, response_style: { ...prev.response_style, tone: e.target.value } }))} className="rounded-2xl border border-white/10 bg-black/45 px-4 py-3 text-sm text-white" /><select value={core.response_style.verbosity} onChange={e => setCore(prev => ({ ...prev, response_style: { ...prev.response_style, verbosity: e.target.value } }))} className="rounded-2xl border border-white/10 bg-black/45 px-4 py-3 text-sm text-white"><option>concise</option><option>balanced</option><option>detailed</option></select><label className="text-[10px] uppercase tracking-widest text-zinc-500">Temperature<input type="number" min={0} max={1.5} step={0.05} value={core.response_style.temperature} onChange={e => setCore(prev => ({ ...prev, response_style: { ...prev.response_style, temperature: Number(e.target.value) } }))} className="mt-2 w-full rounded-2xl border border-white/10 bg-black/45 px-4 py-3 text-sm text-white" /></label><label className="text-[10px] uppercase tracking-widest text-zinc-500">Max Tokens<input type="number" min={100} max={4000} value={core.response_style.max_tokens} onChange={e => setCore(prev => ({ ...prev, response_style: { ...prev.response_style, max_tokens: Number(e.target.value) } }))} className="mt-2 w-full rounded-2xl border border-white/10 bg-black/45 px-4 py-3 text-sm text-white" /></label></div>}{expandedCore === 'behavioral_parameters' && <div className="grid gap-3 sm:grid-cols-2">{Object.entries(core.behavioral_parameters).map(([key, value]) => typeof value === 'boolean' ? <TogglePill key={key} active={value} label={key.replace(/_/g, ' ')} onClick={() => setCore(prev => ({ ...prev, behavioral_parameters: { ...prev.behavioral_parameters, [key]: !value } }))} /> : <label key={key} className="rounded-2xl border border-white/10 bg-black/35 p-4 text-[10px] uppercase tracking-widest text-zinc-500">{key.replace(/_/g, ' ')}<input value={String(value)} onChange={e => setCore(prev => ({ ...prev, behavioral_parameters: { ...prev.behavioral_parameters, [key]: Number.isFinite(Number(e.target.value)) ? Number(e.target.value) : e.target.value } }))} className="mt-2 w-full rounded-xl border border-white/10 bg-black/50 px-3 py-2 text-white" /></label>)}</div>}</div></div><div className="mt-4 rounded-3xl border border-cyan-300/20 bg-black/35 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.3em] text-cyan-300">AI Provider &amp; Model</p><p className="mt-1 text-xs text-zinc-400">Platform providers the server uses when no personal AI Core settings are saved.</p></div><Link to="/casper?settings=ai" className="rounded-2xl border border-cyan-300/30 bg-cyan-400/10 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-cyan-100"><KeyRound className="mr-2 inline h-4 w-4" />Configure Model &amp; API Key</Link></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl border border-white/10 bg-black/45 p-4"><p className="text-[10px] uppercase tracking-widest text-zinc-500">OpenAI-Compatible Endpoint</p><p className="mt-1 truncate text-sm font-bold text-white">{runtime?.ai?.provider_host ?? '—'}</p></div><div className="rounded-2xl border border-white/10 bg-black/45 p-4"><p className="text-[10px] uppercase tracking-widest text-zinc-500">Model</p><p className="mt-1 truncate text-sm font-bold text-white">{runtime?.ai?.model ?? '—'}</p></div><div className="rounded-2xl border border-white/10 bg-black/45 p-4"><p className="text-[10px] uppercase tracking-widest text-zinc-500">Platform API Key</p><p className={cn('mt-1 text-sm font-bold', runtime?.ai?.has_api_key ? 'text-green-200' : 'text-red-200')}>{runtime?.ai ? (runtime.ai.has_api_key ? 'Configured' : 'Missing') : '—'}</p></div><div className="rounded-2xl border border-white/10 bg-black/45 p-4"><p className="text-[10px] uppercase tracking-widest text-zinc-500">Gemini</p><p className={cn('mt-1 text-sm font-bold', runtime?.ai?.gemini_configured ? 'text-green-200' : 'text-zinc-400')}>{runtime?.ai ? (runtime.ai.gemini_configured ? 'Configured' : 'Off') : '—'}</p></div></div><p className="mt-3 text-[11px] text-zinc-500">Routing order differs per surface: tool-calling directives try the OpenAI-compatible endpoint first and fall back to Gemini, while plain text generation tries Gemini first when it is configured. Per-operator provider, endpoint, model, API key, temperature and prompt overrides live in Casper &rarr; AI Core Settings and take priority over both.</p></div></GlassPanel>}

              {activeTab === 'missions' && <GlassPanel><div className="mb-5 flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.3em] text-yellow-200">Autonomous Task Queue</p><h2 className="text-2xl font-black uppercase tracking-[0.12em]">Bot Ops Queue</h2><p className="mt-2 text-xs font-bold uppercase tracking-widest text-zinc-500">Queue worker: {runtime?.queue_busy ? 'executing now' : runtime?.queue_worker ?? 'standby'} · last batch: {runtime?.queue_last_executed ?? 0}/{runtime?.queue_batch_size ?? 0}</p></div><Shield className="h-6 w-6 text-yellow-200" /></div><div className="mb-5 rounded-2xl border border-cyan-300/15 bg-cyan-400/[0.04] p-3 text-[10px] font-bold uppercase tracking-widest text-cyan-100/80">New missions enter `pending` and are claimed automatically by Casper's bot-ops worker. Use Run Now for emergency manual execution.</div><div className="mb-5 grid gap-2 lg:grid-cols-[1fr_1.4fr_auto_auto]"><input value={taskForm.title} onChange={e => setTaskForm(p => ({ ...p, title: e.target.value }))} placeholder="Mission title..." className="rounded-2xl border border-white/10 bg-black/45 px-4 py-3 text-sm text-white" /><input value={taskForm.description} onChange={e => setTaskForm(p => ({ ...p, description: e.target.value }))} placeholder="Mission directive/details..." className="rounded-2xl border border-white/10 bg-black/45 px-4 py-3 text-sm text-white" /><select value={taskForm.priority} onChange={e => setTaskForm(p => ({ ...p, priority: e.target.value as CasperTaskRow['priority'] }))} className="rounded-2xl border border-white/10 bg-black/45 px-4 py-3 text-sm text-white"><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="urgent">Urgent</option></select><button onClick={() => void saveTask()} disabled={!taskForm.title.trim() || saving} className="rounded-2xl border border-yellow-200/30 bg-yellow-500/10 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-yellow-100 disabled:opacity-40">{editingTaskId ? <Save className="inline h-4 w-4" /> : <Plus className="inline h-4 w-4" />} {editingTaskId ? 'Save' : 'Queue'}</button></div><div className="mb-3 flex flex-wrap items-center gap-2">{TASK_STATUS_FILTERS.map(status => { const count = status === 'all' ? tasks.length : tasks.filter(t => t.status === status).length; const isActive = taskStatusFilter === status; return <button key={status} onClick={() => setTaskStatusFilter(status)} className={cn('flex items-center gap-2 rounded-full border px-3 py-1.5 text-[9px] font-black uppercase tracking-widest transition', isActive ? 'border-yellow-200/45 bg-yellow-200/15 text-yellow-100' : 'border-white/10 bg-black/35 text-zinc-500 hover:bg-white/5 hover:text-white')}><span>{status}</span><span className={cn('rounded-md px-1.5 py-0.5 text-[8px]', isActive ? 'bg-yellow-200/20 text-yellow-100' : 'bg-white/5 text-zinc-600')}>{count}</span></button>; })}</div><div className="space-y-3">{filteredTasks.length === 0 ? <div className="rounded-3xl border border-dashed border-white/10 bg-black/35 p-8 text-center"><Shield className="mx-auto mb-3 h-10 w-10 text-zinc-700" /><p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">{tasks.length === 0 ? 'No missions queued. Queue your first directive above.' : `No missions match the \u201C${taskStatusFilter}\u201D filter.`}</p></div> : filteredTasks.map(task => <motion.div key={task.id} layout className="rounded-3xl border border-white/10 bg-black/35 p-4 transition hover:border-cyan-300/25"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-sm font-black uppercase tracking-[0.16em] text-white">{task.title}</p><p className="mt-1 text-xs leading-5 text-zinc-500">{task.description || 'No additional mission notes.'}</p></div><span className={cn('rounded-full border px-3 py-1 text-[8px] font-black uppercase tracking-widest', priorityStyles[task.priority])}>{task.priority}</span></div><div className="mt-3 h-2 rounded-full bg-white/5"><div className="h-full rounded-full bg-cyan-300" style={{ width: `${task.progress ?? (task.status === 'completed' ? 100 : task.status === 'running' ? 50 : 0)}%` }} /></div>{task.result && <div className="mt-3">{(task.metadata as any)?.original_result && (task.metadata as any).original_result !== task.result ? <><p className="rounded-2xl border border-green-300/10 bg-green-400/[0.04] p-3 text-xs leading-5 text-green-100"><span className="mb-1 block text-[9px] font-black uppercase tracking-widest text-green-300">Original Result</span>{(task.metadata as any).original_result}</p><p className="mt-2 rounded-2xl border border-cyan-300/10 bg-cyan-400/[0.04] p-3 text-xs leading-5 text-cyan-100"><span className="mb-1 block text-[9px] font-black uppercase tracking-widest text-cyan-300">Latest Response</span>{task.result}</p></> : <p className="rounded-2xl border border-green-300/10 bg-green-400/[0.04] p-3 text-xs leading-5 text-green-100">{task.result}</p>}{Array.isArray((task.metadata as any)?.followups) && (task.metadata as any).followups.length > 0 && <div className="mt-2 space-y-2 border-l-2 border-cyan-300/20 pl-3">{(task.metadata as any).followups.map((fu: any, idx: number) => <div key={idx}><p className="rounded-xl border border-cyan-300/10 bg-cyan-400/[0.04] p-2 text-[11px] leading-5 text-cyan-100"><MessageSquare className="mr-1 inline h-3 w-3 text-cyan-300" /><span className="font-bold text-cyan-200">You:</span> {fu.question}</p><p className="mt-1 rounded-xl border border-green-300/10 bg-green-400/[0.04] p-2 text-[11px] leading-5 text-green-100"><Ghost className="mr-1 inline h-3 w-3 text-green-300" /><span className="font-bold text-green-200">Casper:</span> {fu.answer}</p></div>)}</div>}{task.status === 'completed' && <div className="mt-2">{followupTaskId === task.id ? <div className="flex gap-2"><input value={followupText} onChange={e => setFollowupText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void sendFollowup(task.id); } }} placeholder="Ask Casper a follow-up question..." className="flex-1 rounded-xl border border-cyan-300/20 bg-black/45 px-3 py-2 text-xs text-white outline-none transition focus:border-cyan-300/50" /><button onClick={() => void sendFollowup(task.id)} disabled={!followupText.trim() || followupLoading} className="rounded-xl border border-cyan-300/20 bg-cyan-400/10 px-3 py-2 text-[8px] font-black uppercase tracking-widest text-cyan-100 disabled:opacity-40">{followupLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : <Send className="h-3 w-3" />}</button><button onClick={() => { setFollowupTaskId(null); setFollowupText(''); }} className="rounded-xl border border-white/10 px-2 py-2 text-zinc-400"><X className="h-3 w-3" /></button></div> : <button onClick={() => { setFollowupTaskId(task.id); setFollowupText(''); }} className="rounded-full border border-cyan-300/15 bg-cyan-400/[0.06] px-3 py-1 text-[8px] font-bold uppercase tracking-widest text-cyan-200 transition hover:bg-cyan-400/15"><MessageSquare className="mr-1 inline h-3 w-3" /> Ask Casper</button>}</div>}</div>}<div className="mt-3 flex flex-wrap items-center justify-between gap-2"><span className={cn('text-[9px] font-black uppercase tracking-widest', statusStyles[task.status])}>{task.status} · {task.status === 'pending' ? 'awaiting auto-run' : task.status === 'running' ? 'worker executing' : task.task_type ?? 'mission'}</span><div className="flex flex-wrap gap-2"><button onClick={() => void runTask(task)} className="rounded-full border border-cyan-300/20 px-3 py-1 text-[8px] uppercase text-cyan-200"><Play className="inline h-3 w-3" /> Run Now</button><button onClick={() => void updateTask(task, { status: 'completed', progress: 100, completed_at: new Date().toISOString(), result: task.result ?? 'Manually completed from GhostOps.' })} className="rounded-full border border-green-300/20 px-3 py-1 text-[8px] uppercase text-green-200">Complete</button><button onClick={() => editTask(task)} className="rounded-full border border-white/10 px-3 py-1 text-[8px] uppercase text-zinc-300"><Edit3 className="inline h-3 w-3" /> Edit</button><button onClick={() => void deleteTask(task)} className="rounded-full border border-red-300/20 px-3 py-1 text-[8px] uppercase text-red-200"><Trash2 className="inline h-3 w-3" /> Delete</button></div></div></motion.div>)}</div></GlassPanel>}
