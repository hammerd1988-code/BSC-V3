...
export const CasperDashboard: React.FC = () => {
  ...
  const updateRoutine = async (routine: CasperRoutineRow, patch: Partial<CasperRoutineRow>) => { const safePatch = { ...patch }; const enabledToggle = safePatch.is_enabled; delete safePatch.is_enabled; const dbPatch = toDb(safePatch); if (enabledToggle !== undefined) dbPatch.enabled = enabledToggle; const { error } = await supabase.from('casper_routines').update(dbPatch).eq('id', routine.id); if (error) setNotice(error.message); else await fetchDashboard(); };
