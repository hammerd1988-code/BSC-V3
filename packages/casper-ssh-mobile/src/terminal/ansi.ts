...
export function parseAnsiLine(input: string): AnsiSpan[] {
  ...
  while ((match = pattern.exec(input))) {
    pushText(input.slice(cursor, match.index).replace(/\r/g, ''));
  ...
  pushText(input.slice(cursor).replace(/\r/g, ''));
  return spans.length ? spans : [{ text: input.replace(CSI_PATTERN, '').replace(/\r/g, ''), style: {} }];
}

export function splitAnsiLines(output: string): string[] {
  return output
    .replace(/\u001b\][^\u0007]*(?:\u0007|\u001b\\)/g, '')
    .split(/\n/)
