# Blood Sweat Code React Native SSH/SFTP

This is a project-owned fork of `@dylankenneally/react-native-ssh-sftp@1.9.0`.
See `NOTICE` for the fork scope and `LICENSE` for upstream licensing.

Android adds OpenSSH `known_hosts` trust-on-first-use, pinned SHA256 host-key
verification, structured host-key errors, base64 raw shell output, and PTY
resize support. iOS keeps the upstream NMSSH behavior where the underlying
NMSSH API does not expose the same SHA256 host-key protocol; callers must
inspect the capability exposed by the app transport.
