...
- `VITE_SUPABASE_ANON_KEY` — token verification. This project uses Supabase's
  newer **publishable** key format (value starts with `sb_publishable_…`) for the
  anon/client role; it is stored in this same env var. Reference the env var by
  name rather than pasting the literal value.
